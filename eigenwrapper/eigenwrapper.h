...
namespace KingOfMonsters {
	...
	public:
		...
		cusolverSpHandle_t& solverSp(int ii, int kk);
	...
	public:
		...
		void _shrinkCols(int M, bool sparse, bool dense);
		...
		void add_usemap(int i, int j, double val);
		...
		int nonzeros();
		void Clear();
		void setmat(Eigen::SparseMatrix<double, Eigen::ColMajor, int>& mat, int ii);
		void setmat(const Eigen::MatrixXd& mat);
		void setmiddlecolum(Eigen::SparseMatrix<double, Eigen::ColMajor, int>& f, int start, int end);
		...
		//std::string _QR_gpu(KingOfMonsters::cuda* cuda, Eigen::MatrixXd* Q, Eigen::MatrixXd* R, int device);
		...
		std::string _solveLU_sparse_gpu(KingOfMonsters::cuda* cuda, Eigen::VectorXd* rhs, Eigen::VectorXd* ret, int device);
